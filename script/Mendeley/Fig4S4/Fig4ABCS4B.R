...
for (Species in Species_l) {
  ...
  mt$C5marker.minus_avg <- scale(mt$C5marker,scale=F)
...
out <- out %>% select(CellID,Species,Stim,MockStim,MonoClusterID,UMAP_1,UMAP_2,C5marker,C5marker.minus_avg,C7marker,C7marker.minus_avg)
