...
data <- data[order(data$SixCellType),]
data$Species_Stim <- factor(data$Species_Stim,levels = SSlevels)
data <- data[order(data$Species_Stim),]
data$SixCellType <- factor(data$SixCellType, levels = Cell_l)
data <- data %>% select(Species,Stim,SixCellType,n2,Freq)
colnames(data) <- c("Species","Stim","CellType","n","Freq")
write.table(data,args[2],sep="\t",quote=F,row.names=F)
