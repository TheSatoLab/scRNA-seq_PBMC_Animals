...
data.m <- data.m[,c("Common","Ra","Mm","RedCommon","RedRa","RedMm")]
data.m <- data.m[data.m$Ra == "ALLhigh",]
ISG <- read.table(args[4],header=T)
data.m$ISG <- rownames(data.m) %in% ISG$gene
...
ISG_l <- c("TRUE" = "black","FALSE" = "gray")
ra <- rowAnnotation(df = data.m,
                    col=list(Commmon = Col_l,Ra = Col_l,Mm = Col_l,
                    RedCommmon = RedCol_l,RedRa = RedCol_l,RedMm = RedCol_l,
                    ISG = ISG_l))
